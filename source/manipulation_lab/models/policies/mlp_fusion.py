...
class MLPFusionOracle(nn.Module):
    def __init__(
            self,
            sequence_length,
            input_dim,
        ...
        self.proprio_input_dim = proprio_input_dim * sequence_length
        self.oracle_input_dim = oracle_input_dim * sequence_length
        ...
        self.oracle_mlp = nn.Sequential(
            nn.Linear(self.oracle_input_dim, 256),
            nn.ReLU(),
            nn.Dropout(p=0.1),
            nn.Linear(256, 128),
            nn.ReLU(),
        ...
        self.proprio_mlp = nn.Sequential(
            nn.Linear(self.proprio_input_dim, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
        )
    ...
    def forward(self, x):
        ...
        proprio = x[:, : self.proprio_input_dim]
        oracle = x[:, self.proprio_input_dim:]

        oracle = self.oracle_mlp(oracle)
        proprio = self.proprio_mlp(proprio)
        fusion = self.fusion_mlp(torch.cat([oracle, proprio], dim=-1))
