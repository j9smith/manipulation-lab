...
@configclass
class LiftBlocksSceneCfg(InteractiveSceneCfg, RoomSceneCfg):
    """
    Design the scene
    """
...
@configclass
class LiftBlocksEnvCfg(DirectRLEnvCfg):
    """
    Create a config for the environment
    """
    sim: SimulationCfg = SimulationCfg()
    decimation: int = 1
    scene: LiftBlocksSceneCfg = LiftBlocksSceneCfg(env_spacing=2.5)
...
class LiftBlocksEnv(DirectRLEnv):
    """
    Load the environment
    """
    cfg: LiftBlocksEnvCfg

    def __init__(self, cfg: LiftBlocksEnvCfg):
        self.mode = cfg.mode
